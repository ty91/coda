...
The repository had design docs but no runnable application scaffold. Milestone 1 required a working CLI shell, app shell, shared core package, and one validation gate that can run locally and in CI.
...
- `apps/app`: Tauri 2 + React/TypeScript shell with health screen, placeholder plan/work panel, and typed frontend-to-Rust command roundtrip (`get_health_message`).
...
- Architecture boundary check: `tools/validate-boundaries.mjs` blocks cross-imports between `apps/cli` and `apps/app`.
...
- Updated `apps/app/src-tauri/tauri.conf.json` bundle target to `app` so packaging passes in this environment.
...
- Keep app bundling target stable (`app`) unless DMG signing/notarization workflow is explicitly configured.
