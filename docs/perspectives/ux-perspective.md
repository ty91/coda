...
This document defines the user experience philosophy, interaction patterns, and design principles for Coda — an agentic software engineering system spanning CLI, Tauri desktop UI, and Slack-based alerts. It draws from the compound engineering loop (Plan → Work → Review → Compound) and the harness engineering principle that **humans steer, agents execute**.
...
$ coda work
  ...
  ○ Task 3/3: Update UI components           ← pending
...
Good:
```
  ✗ Plan file not found: docs/plans/notifications.md
...
- **Section approval**: Each plan section has a ✓/✗ toggle. Approve sections individually or all at once.
- **Rejection with reason**: ✗ always requires a text reason. "Why not?" is the most valuable feedback loop.
...
- **Status bar in terminal**: A persistent one-liner showing fleet summary (e.g., `coda: 3 agents active │ 1 decision waiting │ CI ✓`).
...
As users build confidence in agent output, the system should require fewer interruptions. This is configurable per agent, per project, and per decision type.
...
$ coda init
  ...
  1. Project detection
     ✓ Found: TypeScript project (pnpm, Vitest, ESLint)
     ✓ Found: Git repository (main branch, 847 commits)
  ...
  ✓ Coda is ready. Your first plan is waiting:
...
│           │                                                         │
│           │  Health      CI ✓  │  Tests 847/847  │  Cost $4.20     │
│           │                                                         │
└───────────┴─────────────────────────────────────────────────────────┘
```
...
│    ├ Svc  │                              │    auth-service.ts       │
│    ├ Model│  The service will use the    │    email-service.ts      │
│    ├ API  │  existing event-bus pattern  │    webhook-service.ts    │
│    └ UI   │  to decouple notification    │                          │
│  ○ Tests  │  dispatch from triggers.     │  All use EventBus.emit() │
│  ○ Rollout│                              │  pattern. See line 47 of │
│           │  Methods:                    │  auth-service.ts.        │
│  Legend:  │  • send(user, channel, msg)  │                          │
│  ✓ Approvd│  • getPreferences(user)      │  ─────────────────────── │
│  ~ Review │  • updatePreferences(...)    │                          │
│  ○ Pending│  • batchSend(users, msg)     │  Prior solutions:        │
│           │                              │                          │
...
│           │                              │                          │
├───────────┴──────────────────────────────┴──────────────────────────┤
│  ✓ Goals  │  ✓ Scope  │  ⬜ Impl  │  ⬜ Tests  │  ⬜ Rollout        │
│                                                                     │
...
These are mistakes I've seen repeatedly in developer tools. Coda must not repeat them.
...
| Principle | Meaning |
|-----------|---------|
| **Decisions first** | Pending human decisions are always the most prominent element |
...
| **Consistent vocabulary** | Same words, same colors, same concepts across all surfaces |
| **Earn trust incrementally** | Start supervised, build a track record, offer more autonomy |
| **Respect attention** | Every notification costs focus. Spend that budget wisely |
...
| **The compound test** | Does this UX decision make the next interaction easier? |
