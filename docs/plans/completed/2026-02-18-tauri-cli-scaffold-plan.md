...
- [x] A Tauri desktop package with React/TypeScript frontend and Rust backend shell.
...
1. **Create workspace skeleton (no app logic yet)**
   - [x] Action: Create package folders: `apps/cli`, `apps/desktop`, `packages/core`.
...
6. **Enforce architecture boundaries**
   - [x] Action: Verify dependency direction: `packages/core` consumed by both apps; no cross-import between `apps/cli` and `apps/desktop`.
...
7. **Wire full validation gate**
   ...
   - [x] Deliverables: Add smoke scripts for CLI and desktop startup/build checks.
...
- [x] `pnpm --filter @coda/desktop tauri dev` (manual launch check)
- [x] `pnpm --filter @coda/desktop tauri build` (packaging check)
...
- 2026-02-18: Implemented workspace scaffold (`apps/cli`, `apps/desktop`, `packages/core`) with full validate gate and CI workflow.
